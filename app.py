...
# Load model only when needed (to avoid errors on import)
...
def average_word_length(text):
    ...
    return sum(word_lengths) / len(words) if words else 0
...
def pos_density(text):
    ...
    pos_density = {tag: count / total_words for tag, count in pos_counts.items()} if total_words > 0 else {}
    return pos_density

def sentence_complexity(text):
    sentences = sent_tokenize(text)
    complexity = sum([len(sent.split()) for sent in sentences]) / len(sentences) if sentences else 0
    return complexity

def repetition_ratio(text):
    words = word_tokenize(text)
    unique_words = set(words)
    repetition_ratio = (len(words) - len(unique_words)) / len(words) if words else 0
...
def dynamic_parameter_tracking(transcript_text):
    ...
    readability_score = flesch_kincaid_grade(transcript_text) if transcript_text else 0
